...
summary_stats <- data %>%
  summarise(
    Mean = mean(!!sym(target_col)),
    Median = median(!!sym(target_col)),
    Variance = var(!!sym(target_col)),
    P90 = quantile(!!sym(target_col), 0.90),
    Max = max(!!sym(target_col)),
    Min = min(!!sym(target_col))
  )
...
ggplot(data, aes_string(x = target_col)) +
  ...
  theme_minimal() +
  labs(title = "Histogram", x = target_col, y = "Count")
```
